...
class AuthenticationTabController extends TabController {
	...
	async processActivated () {
		...
		try {
			if (sessionOwner) await this.#invokeUpdatePersonSeat(sessionOwner.identity);
	...
	async #invokeUpdatePersonSeat (personIdentity, seatIdentity = 0) {
		...
		const resource = this.sharedProperties["service-origin"] + "/services/people/" + personIdentity + "/seat";
